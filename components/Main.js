...
const Main = (props) => {
    ...
    let list = notes.map(item => {
        if (item != null) {
            return (<Note color={item.color} header={item.title} content={item.content} id={item.date} date={item.date} key={item.date} fun={getAllItems} />)
        }
    })
    ...
    return (
        <>
            ...
            <Text style={{ textAlign: 'center' }}>Jakub Ciszewski 4ic</Text>
        </>
    )
}
